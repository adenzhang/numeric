...
!r,d,i,c,z,h,l may be followed by v=vector or m=matrix (v,m suﬃxes are used only when
...
END INTERFACE
!Routines returning a location as an integer value (iﬁrstloc, iminloc are not currently over-
...
END INTERFACE
!Routines for skewoperations on matrices (unit matrix, lower triangle, upper triangle not
...
interface get_time
    module procedure sget_time_millisec
end interface
...
FUNCTION ifirstloc(mask)
    !Index of ﬁrst occurrence of .true. in a logical vector.
    LOGICAL(LGT), DIMENSION(:), INTENT(IN) :: mask
...
!Routines for “outer” operations on vectors. The order convention is: result(i,j) = ﬁrst operand(i)
!(op) second operand(j).
...
END FUNCTION vabs
real function sget_time_millisec()
      integer time_array_0(8)
      call date_and_time(values=time_array_0)
      sget_time_millisec = time_array_0 (5) * 3600 + time_array_0 (6) * 60 &
           + time_array_0 (7) + 0.001 * time_array_0 (8)
end function sget_time_millisec
end module
